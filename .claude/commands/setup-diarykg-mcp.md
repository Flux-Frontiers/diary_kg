# DiaryKG MCP Setup & Verification

Set up the DiaryKG MCP server for a target repository and configure it for use with Claude Code and/or Claude Desktop. Execute the following steps in sequence.
...
**Usage:**
- `/setup-diarykg-mcp` — Interactive mode; prompts for the target repository path
- `/setup-diarykg-mcp /path/to/repo` — Set up DiaryKG MCP for the specified repository
...
2. If no argument was provided, ask the user:
   > "Which repository do you want to index with DiaryKG? Please provide the absolute path."
...
- `DB_PATH` → `$REPO_ROOT/.diarykg/graph.sqlite`
- `LANCEDB_DIR` → `$REPO_ROOT/.diarykg/lancedb`

Do not pass `--db` or `--lancedb` flags — the commands default to `.diarykg/` automatically.

---

## Step 1: Verify DiaryKG Installation
...
poetry run diarykg --version 2>&1
```

If that prints a version, set `RUNNER="poetry run"`. If it errors with a Python version
conflict, set `RUNNER=""` and use `.venv/bin/diarykg` directly for all subsequent calls:

```bash
# Fallback — use .venv binary directly
$REPO_ROOT/.venv/bin/diarykg --version
```

Use whichever runner succeeds for all remaining steps. Document which runner was used in
the final report.

1. Check that the `diarykg` entry point resolves:
   ```bash
   $RUNNER diarykg --version   # or $REPO_ROOT/.venv/bin/diarykg --version
   ```
...
1. Check whether `DB_PATH` already exists:
   ```bash
   ls -lh "$REPO_ROOT/.diarykg/graph.sqlite" 2>/dev/null
   ```
2. If it exists, ask the user:
   > "A knowledge graph already exists at `$REPO_ROOT/.diarykg/graph.sqlite`. Rebuild it from scratch (wipe), or keep the existing graph?"
   - **Wipe**: proceed with `--wipe`
   - **Keep**: skip to Step 3

3. Run the static analysis build:
   ```bash
   $RUNNER diarykg build-sqlite --repo "$REPO_ROOT" --wipe
   ```
4. Verify the database was created and is non-empty:
   ```bash
   sqlite3 "$REPO_ROOT/.diarykg/graph.sqlite" "SELECT COUNT(*) FROM nodes; SELECT COUNT(*) FROM edges;"
   ```
...
2. Run a sample query (must be run from `$REPO_ROOT` so `.diarykg/` defaults resolve):
   ```bash
   cd "$REPO_ROOT" && $RUNNER diarykg query --q "module structure"
   ```
...
Configure the per-repo `.mcp.json`, Claude Desktop (`claude_desktop_config.json`) if applicable, and install the DiaryKG skill globally.

The DiaryKG MCP server is started with `diarykg mcp --repo <REPO_ROOT>`. Always use absolute paths.
...
> ⚠️ **Do NOT add `diarykg` to any global settings file.**
...
2. If it exists, check for an existing `diarykg` entry under `servers`.
   - If one exists, ask the user to replace or keep it.

3. The `diarykg` entry to add/update:
   ```json
   {
     "servers": {
       "diarykg": {
         "type": "stdio",
         "command": "<venv_path>/bin/diarykg",
...
2. If it exists, check for an existing `diarykg` entry under `mcpServers`.
   - If one exists, ask the user to replace or keep it.

3. Get the venv binary path:
   ```bash
   poetry env info --path
   # binary: <venv_path>/bin/diarykg
   ```

4. The `diarykg` entry to add/update:
   ```json
   "diarykg": {
     "command": "<venv_path>/bin/diarykg",
     "args": ["mcp", "--repo", "<REPO_ROOT>"]
...
6. **Verify the global settings file does NOT contain a `diarykg` entry:**
   ...
   - If a `diarykg` entry exists in either global file, remove it.
...
### 5d: Install the DiaryKG Skill (Global)

The DiaryKG skill provides AI agents with expert knowledge about DiaryKG installation and usage.
...
Or manually copy from `.claude/skills/diarykg/` to the target directories above.
...
```
✓ DiaryKG version:       <version>
✓ Runner used:          poetry run  OR  .venv/bin/diarykg (fallback)
✓ Repository indexed:   <REPO_ROOT>
✓ SQLite graph:         <REPO_ROOT>/.diarykg/graph.sqlite  (<N> nodes, <M> edges)
✓ LanceDB index:        <REPO_ROOT>/.diarykg/lancedb  (<V> vectors)
✓ Smoke test:           passed
✓ Claude Code config:   <REPO_ROOT>/.mcp.json  (diarykg entry)
✓ Claude Desktop config: <CONFIG_PATH>  (diarykg entry)
✓ DiaryKG skill:         ~/.claude/skills/diarykg/  (installed/updated/skipped)

Restart Claude Code / Claude Desktop to activate the diarykg MCP server.
...
- Prefer `poetry run` for CLI calls; fall back to `.venv/bin/diarykg` if Poetry reports a Python version conflict.
- `mcp` is a **required main dependency** of DiaryKG — there is no `[mcp]` extra to add.
...
| Error | Fix |
|-------|-----|
| `Current Python version is not allowed by the project` | Use `.venv/bin/diarykg` directly instead of `poetry run diarykg` |
| `diarykg: command not found` | Run `poetry install`; if venv exists use `.venv/bin/diarykg` |
| `error: the following arguments are required: --sqlite` | Use `--sqlite`, not `--db`, for `diarykg build-lancedb` |
...
| Empty query results | Run `diarykg build-lancedb --repo "$REPO_ROOT" --wipe` |
| `diarykg mcp` not appearing in Claude Code | Use absolute venv path in `.mcp.json`; restart Claude Code |
