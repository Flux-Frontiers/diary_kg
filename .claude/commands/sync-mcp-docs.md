...
You are updating all DiaryKG MCP documentation and provider instructions to reflect the
current state of `src/diary_kg/mcp_server.py`. Execute the following steps in order.

---

## Step 0: Establish the Source of Truth

Read `src/diary_kg/mcp_server.py` and extract the **authoritative tool list**:
...
### 2b. `.claude/skills/diarykg/SKILL.md`
...
### 2c. `.claude/skills/diarykg/references/CHEATSHEET.md`

This is the portable cheatsheet for use in other repos. It mirrors `docs/CHEATSHEET.md`
but uses generic examples (not diary_kg-specific node IDs where possible).
...
### 2d. `.claude/skills/diarykg/clinerules.md`
...
1. Stage all modified documentation files:
   ...
   git add docs/MCP.md docs/CHEATSHEET.md README.md \
           .claude/skills/diarykg/SKILL.md \
           .claude/skills/diarykg/references/CHEATSHEET.md \
           .claude/skills/diarykg/clinerules.md \
