...
DEFAULT_DIARY = str(Path(__file__).parent.parent.parent / "diary_kg" / "pepys" / "pepys_clean.txt")
...
def embed_local(
    ...
    for batch_start in tqdm(
        ...
        batch_vecs = embedder.encode(prefixed, convert_to_numpy=True, show_progress_bar=False)
...
def main() -> None:  # noqa: C901
    ...
    if not args.no_walker:
        ...
        try:
            ...
            console.print(
                f'  Origin  : entry {i_orig}  ({ts_orig.date()})  "{texts[i_orig][:60]}..."'
            )
            console.print(
