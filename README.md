...
[![Version](https://img.shields.io/badge/version-0.1.0-blue.svg)](https://github.com/Flux-Frontiers/diary_kg/releases)
...
**DiaryKG** — A Deterministic Knowledge Graph for Diary Entries with Semantic Indexing and Source-Grounded Snippet Packing

*Author: Eric G. Suchanek, PhD*

*Flux-Frontiers, Liberty TWP, OH*

[Technical Paper (PDF)](article/diary_kg.pdf)
...
The fastest way to integrate DiaryKG into any Python repository — run the one-line installer from within the repo you want to index:
...
diarykg explain "fn:src/diary_kg/store.py:GraphStore.expand"
```
...
kg = DiaryKG(repo_root="/path/to/repo")
...
pip install 'code-kg @ git+https://github.com/Flux-Frontiers/diary_kg.git'

# With Streamlit web visualizer (adds Streamlit, pyvis, plotly)
pip install 'code-kg[viz] @ git+https://github.com/Flux-Frontiers/diary_kg.git'

# With 3D visualizer extras (adds PyVista, PyQt5, etc. — heavy dependencies)
pip install 'code-kg[viz3d] @ git+https://github.com/Flux-Frontiers/diary_kg.git'

# With both visualizers
pip install 'code-kg[viz,viz3d] @ git+https://github.com/Flux-Frontiers/diary_kg.git'
```
...
poetry add 'code-kg @ git+https://github.com/Flux-Frontiers/diary_kg.git'

# With Streamlit web visualizer (adds Streamlit, pyvis, plotly)
poetry add 'code-kg[viz] @ git+https://github.com/Flux-Frontiers/diary_kg.git'

# With optional viz3d extras (for 3D visualizer — does NOT auto-install in CI)
poetry add 'code-kg[viz3d] @ git+https://github.com/Flux-Frontiers/diary_kg.git'
```
...
# Core (no visualizer)
code-kg = {git = "https://github.com/Flux-Frontiers/diary_kg.git"}

# With Streamlit visualizer
code-kg = {git = "https://github.com/Flux-Frontiers/diary_kg.git", extras = ["viz"]}

# With 3D visualizer
code-kg = {git = "https://github.com/Flux-Frontiers/diary_kg.git", extras = ["viz3d"]}
```

> **Note for CodeKG developers:** When working on CodeKG itself, use `poetry install -E viz` to install the Streamlit visualizer, or `poetry install --with viz3d` for the 3D visualizer dependencies. The `extras` mechanism above is for *consumers* of the package; the `-E` / `--with` flags are for local development only.
...
```toml
[tool.poetry.scripts]
my-build-sqlite  = "diary_kg.cli.cmd_build:build_sqlite"  # ✅ Correct
my-build-lancedb = "diary_kg.cli.cmd_build:build_lancedb"  # ✅ Correct
my-mcp           = "diary_kg.mcp_server:main"              # ✅ Correct
```
...
```toml
[tool.poetry.scripts]
my-build-sqlite = "diary_kg.build_codekg_sqlite:main"  # ❌ Wrong (legacy)
```

The legacy module names (`build_codekg_sqlite`, `build_codekg_lancedb`) exist only for backward compatibility as re-export stubs. Always import from `diary_kg.cli.cmd_build` instead.
...
│   ├── CHEATSHEET.md
│   ├── diary_kg.pdf
│   ├── diary_kg_arch_9x16.png
│   ├── diary_kg_arch_banana.png
│   ├── diary_kg_arch_square.png
│   ├── diary_kg_workflow.md
│   ├── deployment.md
│   ├── MCP.md
│   └── logo.png
├── lib/
...
cd diary_kg
poetry install          # core + dev dependencies
poetry install -E viz   # add Streamlit visualizer
```
