...
3. **Captures:** Temporal snapshot with current metrics:
   ...
   - Complexity hotspots
   - Critical issues
4. **Commits:** Snapshot back to `.diarykg/snapshots/` in the repository
...
### In Git History
Snapshots are committed to `.diarykg/snapshots/{commit}.json`:

```bash
git log --oneline -- .diarykg/snapshots/
```
...
3. Download snapshots artifact (`.diarykg/snapshots/`)
...
diarykg snapshot show <commit>

# Compare two snapshots
diarykg snapshot diff <commit-a> <commit-b>
```
...
Only changes to `.diarykg/snapshots/` are committed:
- `.diarykg/snapshots/{commit}.json` — Full snapshot
- `.diarykg/snapshots/manifest.json` — Index of all snapshots
...
```
✓ Snapshot saved: .diarykg/snapshots/dd1b9d3a707ab18c22bdc9963ed42bd8930f2747.json
...
✓ Snapshot committed to .diarykg/snapshots/
```
