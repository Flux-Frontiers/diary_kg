#!/usr/bin/env python3
"""
Generate GitHub Wiki pages for the diary_kg repository.
...
Usage:
    ...
    python scripts/generate_wiki.py --repo Flux-Frontiers/diary_kg
...
def generate_home_page(readme_path: Path, logo_path: Path | None = None) -> str:
    ...
    if logo_path and logo_path.exists():
        logo_section = f"![DiaryKG Logo]({logo_path.name})\n\n"

    wiki_header = f"""{logo_section}# DiaryKG Wiki
...
def generate_installation_page(readme_path: Path) -> str:
    ...
    if section is None:
        return (
            "# Installation\n\n"
            "See [README.md](https://github.com/Flux-Frontiers/diary_kg/blob/main/README.md) "
...
This page documents the public Python API for the `diary_kg` package.
...
- [GitHub Repository](https://github.com/Flux-Frontiers/diary_kg)
- [Medium Article](https://medium.com/@coder/diary-kg)
"""
    return sidebar
...
def clone_wiki(repo_url: str, temp_dir: str) -> None:
    ...
    :param repo_url: GitHub repository slug (e.g. ``'Flux-Frontiers/diary_kg'``).
...
def main() -> None:
    ...
    parser.add_argument(
        "--repo",
        default="Flux-Frontiers/diary_kg",
        help="GitHub repository slug (default: Flux-Frontiers/diary_kg)",
    )
